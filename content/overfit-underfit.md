...
ここで重要なのは、誤差を一つの時点の値として眺めるだけでなく、容量を変えたときに二つの誤差がどう動くかを対応づけることです。訓練誤差が下がったという事実は、訓練データへの適合が進んだことしか示しません。テスト誤差も合わせて見れば、その適合が未知データにも役立っているのか、それとも訓練データだけに偏っているのかを区別できます。
...
つまり、同じ「テスト誤差が高い」という観察でも、訓練誤差まで高ければ未学習、訓練誤差だけが低ければ過学習というように読み方が変わります。モデルを大きくすることは訓練データへの適合を改善しやすい一方、未知データでの改善を保証しません。二つの曲線を同じ条件で比べることで、容量を増やす判断と抑える判断を取り違えにくくなります。
...
対処を選ぶときは、まず誤差の組合せから容量の側を確かめます。未学習なら表現できる幅が足りないため、正則化でさらに制約すると訓練データへの適合も妨げます。過学習なら訓練データへの適合をそのまま強めるのではなく、データ、モデルの容量、重み、ユニット、学習時間のいずれかを制約する方向にそろえます。この順序で考えると、同じ手法を状態に関係なく適用する誤りを避けられます。
...
この比較では、次数を変えることがモデルの容量を変える操作に対応しています。次数が低い場合は訓練データの傾向を十分に表せず、次数を上げると訓練誤差は下がります。しかし、テスト誤差まで同じように下がるとは限らず、訓練誤差との開きが現れたところで過学習側の特徴が確認できます。したがって、出力の大小を単独で評価せず、訓練とテストの組合せとして読むことがこのコードの確認点です。
