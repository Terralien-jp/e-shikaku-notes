...
ここでいう「近さ」は、単語を人手で分類して与える属性ではありません。文中でどの語と一緒に現れ、どの語を予測する必要があるかという関係が、学習された座標の差として表れます。そのため、埋め込みは単語を識別するための記号を、後の計算で扱える連続的な表現へ置き換える役割を持ちます。
...
埋め込み行列を $E \in \mathbb{R}^{V\times d}$、埋め込み次元を $d$ とすると、単語 $w$ のベクトルは one-hot ベクトル $\mathbf{x}_w$ に対して $\mathbf{e}_w=E^{\mathsf T}\mathbf{x}_w$ と取り出せます。通常は $d\ll V$ です。この取り出しは、one-hot の1が立った行を埋め込み行列から選ぶ操作と同じであり、単語を高次元の疎な入力のまま扱い続けるのではなく、学習対象の行へ移す段階です。学習後は cosine 類似度
...
とします。$w_i$ は入力語、$w_o$ は正解の出力語、$V$ は語彙数です。正解の周辺語の対数確率を最大化すると、同じ文脈で予測される語の内積が上がり、共起のパターンが埋め込みに分散して入ります。つまり、入力語のベクトルだけを単独で決めるのではなく、出力側の語との組み合わせが予測しやすくなるように、両側のベクトルが更新されます。CBOW では入力側の周辺語ベクトルを平均して $\mathbf{e}_{\mathrm{ctx}}$ とし、同じ形式で中心語を予測します。周辺語を平均するため、表に示した通り文脈内での順序はこの基本形では使われません。
