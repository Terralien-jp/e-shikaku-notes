...
|用語|単語埋め込みとの違い|
|---|---|
|one-hot 表現|語彙の各語に固有の座標を割り当てる疎表現。類似度を学習していない|
|分散表現|複数の次元に情報を分散して持たせる考え方。word2vec はその学習方法の一つ|
|CBOW|周辺語から中心語を予測するモデル。埋め込みそのものの別名ではない|
|Skip-gram|中心語から周辺語を予測するモデル。入力と出力のどちらの行列を使うかも実装で確認する|
|文脈依存表現|同じ単語でも文脈でベクトルが変わる。word2vec 系の静的な埋め込みとは扱う情報が違う|
