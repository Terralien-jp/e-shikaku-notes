...
この違いは、推定値を得た後に何をするかにも現れます。点推定では選んだ値以外の候補を計算から外しますが、事後分布では候補ごとの重みを残したまま扱えます。データが少ないときに一つの値へ急いで固定しないことが、ベイズ推論で不確実性を保持するという意味です。
...
ここで、尤度と事前分布は同じ役割ではありません。尤度は観測したデータがパラメータごとにどれだけ整合するかを表し、事前分布はデータを見る前の重みを表します。両者を掛け合わせた結果を全域で正規化することで、データを見た後のパラメータの重みとして読み直せます。
...
コードの各行は、式のどの部分を計算しているかに対応しています。prior が事前分布、likelihood が観測結果による重み付け、posterior の正規化が事後分布の作成です。最後の積分では、パラメータの値そのものに事後分布の重みを掛け、候補を一つに絞らずに平均しています。
